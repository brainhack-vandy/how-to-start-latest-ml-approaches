...
def get_args():
    parser = argparse.ArgumentParser('NeuroBOLT intra- and inter-suject training script', add_help=False)
    ...
    # Model parameters
    parser.add_argument('--model', default='neurobolt_default', type=str, metavar='MODEL',
    ...
    parser.add_argument('--log_dir', default='./log/neurobolt_default/',
    ...
    # ---- Dataset params ----
    parser.add_argument(
        "--atlas", type=str, default="Difumo", help="atlas used for extracting roi"
    )
    parser.add_argument('--nb_roi', default=1, type=int,
                        help='number of the output ROI')
    parser.add_argument(
        "--labels_roi", type=str, default='Thalamus', help="fmri ROI name in str"
                                                         "Cuneus|Heschl’s gyrus|"
                                                         "Middle frontal gyrus anterior|"
                                                         "Precuneus anterior|"
                                                         "Putamen|Thalamus|"
                                                         "global signal clean| or other ROI name in atlas"
    )
    ...
    # parser.add_argument('--prepro_datapath', default='./vu_seq2one_full_16s_Cuenus.pkl', type=str,
    #                     help='path to the preprocessed epoch datasets')
    parser.add_argument('--prepro_datapath', default=None, type=str,
                        help='path to the preprocessed epoch datasets: Cuneus | Hesch | Mid_fron_gyr_ant |'
                             'Precuneus_anterior | Putamen | Thalamus | glb')
    parser.add_argument('--save_input_tensor', action='store_true', default=False,
                        help='Flag to save the input tensor when do inter-subject training.')
    parser.add_argument('--train_test_mode', default='intrascan', type=str,
                        help='intrascan/full_test/full_retainvu/')
    parser.add_argument('--split_index_sheet', default='./scan_split.xlsx', type=str,
                        help='Path to the Excel sheet specifying train-test split indices when do cross-subject training')

    # ---- Dataset specific info ----
    parser.add_argument('--TR', default=2.1, type=float,
                        help='the TR value (Repetition Time) for fMRI collection in seconds')
    parser.add_argument('--mri_sync_event', default='R149', type=str,
                        help='the event name in EEG data to indicate the time for MRI scan collection')
    parser.add_argument('--nb_eegchan', default=26, type=int,
    ...
    parser.add_argument(
        "--dataname", type=str, default="sub11-scan01", help="data name with sub and scan for loading data for "
                                                              "intra-scan training ")
...
def get_dataset(args):
    ...
    if args.train_test_mode == 'intrascan':
        train_dataset, test_dataset, val_dataset = get_datasets.prepare_onesub_dataloader(args)

        if args.dataset == 'VU':
            ch_names = ['FP1', 'FP2', 'F3', 'F4', 'C3', 'C4', 'P3', 'P4', 'O1', 'O2', 'F7', 'F8', 'T7', 'T8',
                        'P7', 'P8', 'FPZ', 'FZ', 'CZ', 'PZ', 'POZ', 'OZ', 'FT9', 'FT10', 'TP9', 'TP10']

    elif args.train_test_mode == 'full_test':
        train_dataset, test_dataset, val_dataset = get_datasets.prepare_full_dataloader(args)
        if args.dataset == 'VU':
            ch_names = ['FP1', 'FP2', 'F3', 'F4', 'C3', 'C4', 'P3', 'P4', 'O1', 'O2', 'F7', 'F8', 'T7', 'T8',
                        'P7', 'P8', 'FPZ', 'FZ', 'CZ', 'PZ', 'POZ', 'OZ', 'FT9', 'FT10', 'TP9', 'TP10']
        ...
        train_dataset, test_dataset, val_dataset = get_datasets.prepare_full_dataloader(args)
        ch_names_full_vu = ['FP1', 'FP2', 'F3', 'F4', 'C3', 'C4', 'P3', 'P4', 'O1', 'O2', 'F7', 'F8', 'T7', 'T8',
                            'P7', 'P8', 'FPZ', 'FZ', 'CZ', 'PZ', 'POZ', 'OZ', 'FT9', 'FT10', 'TP9', 'TP10']
        # Here, if you are using the pretrained weights of neurobolt trained on VU dataset, ch_names should be the
        # subset of full_vu channels. Below is an example channel set, you could modify it to adapt your own dataset
        ch_names = ['FP1', 'FP2', 'F3', 'F4', 'C3', 'C4', 'P3', 'P4', 'O1', 'O2', 'F7', 'F8', 'T7', 'T8',
    ...
    if ch_names:
        ...
        raise ValueError(
            "Channel names are missing. Please provide channel names for your own dataset in the 'ch_names' variable.")
