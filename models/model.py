...
NeuroBOLT has two modules:
- 1) Spatiotemporal representation learning module - based on a pretrained EEG foundation model (LaBraM-base)
- 2) Multi-scale spectral representation learning module

The model is built upon the LaBraM, BIOT, BEiT-v2, timm, DeiT, and DINO codebases:
...
class NeuroBOLTransformer(nn.Module):
    def __init__(self, EEG_length=3200, EEG_channel=26, patch_size=200, in_chans=1, out_chans=8, num_roi=1, embed_dim=200, win_level=3,
        ...
        #                                                                                       in_chans=in_chans,
        #                                                                                       embed_dim=embed_dim)
        self.patch_embed = TemporalConv(out_chans=out_chans) if in_chans == 1 else PatchEmbed(patch_size=patch_size,
                                                                                              in_chans=in_chans,
                                                                                              embed_dim=embed_dim)
    ...
    def forward_ts_features(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
    ...
    def forward(self, x, input_chans=None, input_chans_spect=None, return_patch_tokens=False, return_all_tokens=False,
                **kwargs):
        '''
        x: [batch size, number of electrodes, number of patches, patch size]
        For example, for an EEG sample of 16 seconds with 64 electrodes, x will be [batch size, 64, 16, 200]
        '''
