...
class MSSEncoder(nn.Module):
    def __init__(
        ...
        #     # [PatchWindowEmbedding(emb_size=32, n_freq=int(self.input_length / (100 * (2 ** i))))
        #     [PatchWindowEmbedding(emb_size=int(self.input_length/self.scale1), n_freq=int(self.input_length / (self.scale1 * (2 ** i))))
        #
        #      for i in range(self.win_level + 1)]
        # )
    ...
    def forward(self, x, input_chans=None, n_channel_offset=0, perturb=False):
        ...
        for i in range(x.shape[1]):
            ...
            for n in range(self.win_level + 1):
                channel_spec_emb_list.append(self.ts_embedding[n](
                    self.patch_embedding[n](
                        # self.stft(x[:, i: i + 1, :], n_fft=100 * (2 ** n)))))
            ...
            # (batch_size, ts, emb)
            # channel_emb = self.positional_encoding(channel_spec_emb + channel_token_emb.to('cuda'))
